...
def closePDF(pdf,title='Plots',author='Debashis Sahoo',subject='Bone',keywords='disease training validation ROC',
             creationYear=2021,creationMonth=10,creationDay=18):
    import datetime
    d = pdf.infodict()
    d['Title'] = title
    d['Author'] = author
    d['Subject'] = subject
    d['Keywords'] = keywords
    d['CreationDate'] = datetime.datetime(creationYear,creationMonth,creationDay)
...
class IBDAnalysis:
    ...
    def orderDataDf(self, gene_groups, weight,urlbase='http://hegemon.ucsd.edu/Tools/explore.php'):
        ...
        for k in gene_groups:
            df_g = hu.getHegemonGeneIDs(self.dbid, k,urlbase=urlbase)
    ...
    def getPvals(self):
        lval, score = self.getScores(None)
        pvals = []
        for i in range(1, len(lval)):
            if len(lval[i]) <= 0:
                pvals += [""]
